...
public class UserController {
	...
	@GetMapping("/user/viewjob")
	public String userViewJob(HttpSession session,Model model) {
	...
	public String changePwd(HttpSession session,HttpServletRequest request,RedirectAttributes attrib) {
		...
		try {
			...
			attrib.addFlashAttribute("msg", "UserId is not amtch");
			return "redirect:/user/changepwd";
		}
	...
	public String giveResponse(HttpSession session,@ModelAttribute ResponseDto rdto,RedirectAttributes attrib) {
		...
		String postdate  = sdf.format(dt);
		response.setPostdate(postdate);
		User user = (User) session.getAttribute("user");
		response.setName(user.getName());
		response.setContactno(user.getContactno());
		rrepo.save(response);
		attrib.addFlashAttribute("msg", "Your response is submittes");
	...
	public String appliedJob(@PathVariable("id") int jobid,HttpSession session,RedirectAttributes attrib) {
		if (session.getAttribute("user") == null) {
			return "redirect:/login";
		}
		User user = (User) session.getAttribute("user");
		String emailaddress = user.getEmailaddress();
		if(ajrepo.existsByJobidAndEmailaddress(jobid, emailaddress)==true) {
			attrib.addFlashAttribute("msg", "You have already applied for this job.");
			return "redirect:/user/viewjob";
		}
		JobInfo ji = jrepo.findById(jobid).get();
		Appliedjob aj = new Appliedjob();
		aj.setJobid(ji.getId());
		aj.setTitle(ji.getTitle());
		aj.setDescription(aj.getDescription());
		...
		aj.setStatus("true");
		ajrepo.save(aj);
		attrib.addFlashAttribute("msg", "Applied successfully ");
		return "redirect:/user/viewjob";
	}
}
